...
prepareIndexes <- function(res){
  ...
  stopifnot("res must be a DataFrame"= class(DataFrame(res)) == "DFrame")
...
cleanHDX <- function(res, clean = TRUE){
  ...
  if(isTRUE(clean)){
    ...
    # check duplications
    duplicated_rows <- duplicated(res[, c("State",
                                          "Sequence",
                                          "replicate",
                                          "Exposure")])
    ...
    missing_seqs <- res[!complete.cases(res), "Sequence"]
