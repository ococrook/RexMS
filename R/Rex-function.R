...
rex <- function(HdxData,
  ...
  # checks
  stopifnot(exprs = {"HdxData must be a DataFrame"= is(HdxData, "DFrame")
  ...
  # check the priors
  stopifnot(exprs = {"lambda must be a numeric" = is.numeric(priors$lambda)
  ...
  .out <- .RexParams(method = "ReX",
                     priors = priors,
                     seed = as.integer(seed),
                     summary = .RexSummary(summary = .RexSummary()),
                     chains = .chains,
...
##' @param range The iterations to keep
...
RexProcess <- function(HdxData,
  ...
  # checks
  stopifnot(exprs = {"params must be a RexParams"= is(params, "RexParams")
                     "burnin must be a numeric" = is.numeric(burnin)
                     "thin must be a numeric" = is.numeric(thin)})
  
  # check the chains
  stopifnot(exprs = {"chains must be a RexChains"= is(params@chains, "RexChains")
                     "summary must be a RexSummary"= is(params@summary, "RexSummary")
                     "burnin must be a numeric"= is.numeric(burnin)
                     "thin must be a numeric"= is.numeric(thin)})
  
  # check the range
  stopifnot(exprs = {"range must be a numeric"= is.numeric(range)})
  
  ## storage
  mean_blong <- matrix(nrow = numChains, ncol = R)
  mean_pilong <- matrix(nrow = numChains, ncol = R)
  mean_qlong <- matrix(nrow = numChains, ncol = R)
  mean_dlong <- matrix(nrow = numChains, ncol = R)
  C <- matrix(NA, nrow = R, ncol = numPeptides)
  
  
  ## sigma
  sigma <- vector(mode = "numeric", length = numChains)
  likelihood <- vector(mode = "numeric", length = numChains)

  ## quantiles
  .blong_quantiles <- matrix(nrow = numChains, ncol = 3)
  .pilong_quantiles <- matrix(nrow = numChains, ncol = 3)
  .qlong_quantiles <- matrix(nrow = numChains, ncol = 3)
  .dlong_quantiles <- matrix(nrow = numChains, ncol = 3)
  
  
  for (j in seq_len(numChains)){
    mean_blong[j,] <- colMeans(params@chains[[j]]@blong[, range])
    mean_pilong[j,] <- colMeans(params@chains[[j]]@pilong[, range])
    mean_qlong[j,] <- colMeans(params@chains[[j]]@qlong[, range])
    mean_dlong[j,] <- colMeans(params@chains[[j]]@dlong[, range])
  ...
  .blong_quantiles <- vapply(seq_len(R),
                           function(j) quantile(params@chains[[k]]@blong[j, range],
                                                c(0.025, 0.5, 0.975)),
                           FUN.VALUE = numeric(3))
  .pilong_quantiles <- vapply(seq_len(R),
                           function(j) quantile(params@chains[[k]]@pilong[j, range],
                                                c(0.025, 0.5, 0.975)),
                           FUN.VALUE = numeric(3))
  .qlong_quantiles <- vapply(seq_len(R),
                           function(j) quantile(params@chains[[k]]@qlong[j, range],
                                                c(0.025, 0.5, 0.975)),
                           FUN.VALUE = numeric(3))
  .dlong_quantiles <- vapply(seq_len(R), 
                           function(j) quantile(params@chains[[k]]@dlong[j, range],
                                                c(0.025, 0.5, 0.975)),
                           FUN.VALUE = numeric(3))
  }
  
  ## take means across chains
  blong <- rowMeans(mean_blong)
  pilong <- rowMeans(mean_pilong)
  qlong <- rowMeans(mean_qlong)
  dlong <- rowMeans(mean_dlong)
  
  .diagnoistics <- matrix(NA, 1, 1)
  if(numChains > 1){
    sigma_mcmc <- coda::as.mcmc(sigma)
    gd <- coda::gelman.diag(sigma_mcmc)
    .diagnoistics <- c(gd$psrf)
    rownames(.diagnoistics) <- c("sigma")
  }

  err <- error_prediction(res = HdxData,
                          blong = blong[j,],
                          pilong = pilong[j,],
                          qlong = qlong[j,],
                          dlong = dlong[j,],
  ...
  Rex.resolution <- DataFrame(cbind(Residues = Residues,
                                    signedARE = signedARE,
                                    ARE = ARE,
                                    TRE = TRE))
  
  Rex.peptideError <- DataFrame(cbind(unique(HdxData$Sequence),
                                      err))
  
  colnames(Rex.peptideError) <- c("Peptide", timepoints[-1])
                                
  
  colnames(Rex.solution) <- c("Resdiues",
    paste0(rep(c("signedARE_", "ARE_", "TRE_"), each = numTimepoints - 1), timepoints[-1]))
  



  posteriorEstimates <- DataFrame(Residues = Residues,
                                  blong = blong,
                                  pilong = pilong,
                                  qlong = qlong,
                                  dlong = dlong)
  
  colnames(posteriorEstimates) <- c("Residues",
                                    "blong",
                                    "pilong",
                                    "qlong",
                                    "dlong")
  ...
  Rex.quantiles <- DataFrame(blong_quantiles = .blong_quantiles,
                             pilong_quantiles = .pilong_quantiles,
                             qlong_quantiles = .qlong_quantiles,
                             dlong_quantiles = .dlong_quantiles)
  
  colnames(Rex.quantiles) <- c("blong_quantiles",
                               "pilong_quantiles",
                               "qlong_quantiles",
                               "dlong_quantiles")
  
  rownames(Rex.quantiles) <- c("2.5%", "50%", "97.5%")
  
  Rex.globals <- DataFrame(sigma = sigma,
                           likelihood = likelihood)
  colnames(Rex.globals) <- c("sigma", "likelihood")
  

  ## store the summary
  params@summary <- RexSummary(posteriorEstimate = posteriorEstimate,
