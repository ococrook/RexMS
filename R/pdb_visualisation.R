...
hdx_to_pdb_colours <- function(dataset,
    ...
    # logging
    rlog::log_info(paste(
        " Your HDX input dataset has",
        length(colnames(dataset)), "entries"
    ))
    rlog::log_info(paste(
        " And excluding NA data you only have",
        length(dataset[!is.na(dataset)]), "entries"
    ))
    rlog::log_info(paste(
        " However, your input PDB has only",
    ...
    residue_numbers_hdx_pdb <- dplyr::intersect(
        sequence_residue_numbers_hdx,
    ...
    pdb_viewer_data <- list(
        "values" = dataset_for_pdb_mapping,
    ...
    df <- data.frame(
        x = unlist(lapply(pdb_viewer_data$values, color_function)),
