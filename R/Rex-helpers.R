  ...
  for (j in seq_len(R)){
    ...
    if (sum(is.na(data)) == 0){
      ...
      if (numtimepoints > 4){
        ...
        out <- try(nlsLM(data = as.data.frame(data),
...
##' @param numtimepoints The number of timepoints
##' @param R The number of residues
##' @param phi The maximum uptake possible
...
uptakeGuess <- function(res,
  ...
  uptake_guess <- lapply(uptake_guess, function(X) {X[X > phi] <- phi
  return(X)})
  uptake_guess <- lapply(uptake_guess, function(X) {X[X < 0] <- 0
  return(X)})
