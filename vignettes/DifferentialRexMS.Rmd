...
initial steps here to focus on the aspects inmportant for differential analysis.
...
We actually only need to run `rex` explicitly on condition we are primarily
interested in e.g. 'ligand' vs 'apo', we can only need to model the first 
dataset initially. If you swap the analysis around are start with `apo` first
your results will be inverted (e.g. report protection instead of deprotection)
as a consequence of the data. If you need to seperate our conditions from
...
Chain 1 had a higher log-likelihood in the ligand bound state so we will use
...
using the timeaveraged probabilities.
